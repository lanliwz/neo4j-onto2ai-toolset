...
class FallbackBehavior(Enum):
    """Enumeration of fallback behaviors applied when a column mask rule cannot resolve masking inputs."""
    BLOCK_QUERY = "block query"
    RETURN_NULL = "return null"
    USE_DEFAULT_MASK = "use default mask"

class DenyBehavior(Enum):
    """Enumeration of enforcement behaviors applied when a row filter rule denies access."""
    BLOCK_QUERY = "block query"
    RETURN_NO_ROWS = "return no rows"

class ValueSourceType(Enum):
    """Enumeration of runtime source categories used to resolve entitlement rule values."""
    DERIVED_QUERY = "derived query"
    REQUEST_CONTEXT = "request context"
    SESSION_CONTEXT = "session context"
    STATIC_LITERAL = "static literal"
    SUBJECT_ATTRIBUTE = "subject attribute"
...
class MaskingMethod(Enum):
    """Enumeration of masking or transformation strategies available to column mask rules."""
    HASHING = "hashing"
    NULL_REPLACEMENT = "null replacement"
    PATTERN_MASKING = "pattern masking"
    STATIC_SUBSTITUTION = "static substitution"
...
class MaskAction(Enum):
    """Enumeration of actions a column mask rule can apply to protected column values."""
    NULLIFY = "nullify"
    REDACT = "redact"
    REVEAL = "reveal"
...
class MatchMode(Enum):
    """Enumeration of value cardinality modes expected by a row filter rule."""
    MULTIPLE_VALUES = "multiple values"
    NO_VALUE = "no value"
    SINGLE_VALUE = "single value"

class ComparisonOperator(Enum):
    """Enumeration of comparison operators available to row filter predicates."""
    BETWEEN = "between"
    EQUALS = "equals"
    IN_LIST = "in list"
...
class FilterAction(Enum):
    """Enumeration of actions a row filter rule can apply during entitlement evaluation."""
    ALLOW = "allow"
    DENY = "deny"
...
class User(BaseModel):
    ...
    has_user_type: UserType = Field(alias="hasUserType", description="A user is classified by a user type that identifies whether it is a human actor or an automated process.")
    is_member_of: List[PolicyGroup] = Field(default_factory=list, min_length=1, alias="isMemberOf", description="User inherits policies via policy group membership.")
...
class EntitlementRule(BaseModel):
    ...
    has_priority: RulePriority = Field(alias="hasPriority", description="A row filter rule or column mask rule is assigned a priority level for evaluation order.")
    has_value_source_type: Optional[ValueSourceType] = Field(default=None, alias="hasValueSourceType", description="Associates an entitlement rule with the runtime source category used to resolve rule values.")
...
class RowFilterRule(EntitlementRule):
    ...
    has_comparison_operator: Optional[ComparisonOperator] = Field(default=None, alias="hasComparisonOperator", description="Associates a row filter rule with the comparison operator used in its predicate.")
    has_deny_behavior: Optional[DenyBehavior] = Field(default=None, alias="hasDenyBehavior", description="Associates a row filter rule with the enforcement behavior used when access is denied.")
    has_filter_action: Optional[FilterAction] = Field(default=None, alias="hasFilterAction", description="Associates a row filter rule with the action it applies when rewriting or evaluating a query.")
    has_match_mode: Optional[MatchMode] = Field(default=None, alias="hasMatchMode", description="Associates a row filter rule with the value cardinality mode it expects.")
    targets_filtered_column: List[Column] = Field(default_factory=list, min_length=1, alias="targetsFilteredColumn", description="Row-filter rule targets a specific column context.")
...
class ColumnMaskRule(EntitlementRule):
    ...
    has_fallback_behavior: Optional[FallbackBehavior] = Field(default=None, alias="hasFallbackBehavior", description="Associates a column mask rule with the fallback behavior used when masking inputs cannot be resolved.")
    has_mask_action: Optional[MaskAction] = Field(default=None, alias="hasMaskAction", description="Associates a column mask rule with the masking action it applies.")
    has_masking_method: Optional[MaskingMethod] = Field(default=None, alias="hasMaskingMethod", description="Associates a column mask rule with the masking or transformation method it uses.")
    targets_masked_column: List[Column] = Field(default_factory=list, min_length=1, alias="targetsMaskedColumn", description="Column-mask rule targets a specific column.")
...
class Table(BaseModel):
    ...
    belongs_to_schema: Schema = Field(alias="belongsToSchema", description="A table belongs to exactly one schema.")
...
class Policy(BaseModel):
    ...
    policy_description: List[str] = Field(default_factory=list, alias="policyDescription", description="A textual description of the purpose, scope, or semantics of a policy.")
    policy_id: str = Field(alias="policyId", description="The unique identifier assigned to a policy.", json_schema_extra={"unique": True})
    policy_name: List[str] = Field(default_factory=list, alias="policyName", description="The human-readable name assigned to a policy.")
...
class Schema(BaseModel):
    ...
    belongs_to_database: RelationalDatabase = Field(alias="belongsToDatabase", description="Schema belongs to a relational database.")
...
class PolicyGroup(BaseModel):
    ...
    policy_group_name: List[str] = Field(default_factory=list, alias="policyGroupName", description="The human-readable name assigned to a policy group.")
    includes_policy: List[Policy] = Field(default_factory=list, min_length=1, alias="includesPolicy", description="Policy group bundles one or more policies.")
...
class Column(BaseModel):
    ...
    belongs_to_table: Table = Field(alias="belongsToTable", description="A column belongs to exactly one table.")
...
class JdbcConnectionProfile(BaseModel):
    ...
    connects_to: RelationalDatabase = Field(alias="connectsTo", description="JDBC profile connects to a target relational database.")
