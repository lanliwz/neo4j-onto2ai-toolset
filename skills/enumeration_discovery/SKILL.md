...
Use these instructions to automatically identify which `owl:Class` nodes in the `stagingdb` should be treated as enumerations in diagrams and code generation.

## 1. Discovery Logic
An enumeration is defined as an `owl:Class` that has one or more `owl:NamedIndividual` members linked via `rdf:type`.
