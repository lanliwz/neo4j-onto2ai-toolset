...
def create_entitlement_model_prompt(
    ...
    prompt = (
        ...
        "- policy — Encapsulates access logic, combining row-level and column-level rules. Each policy must has a unique required policy_id "
        "and a policy_name, optionally has a definition\n"
        "- policy group — A collection of policies aligned to a persona, function, or role set. Each policy must has a unique required "
        ...
        "- user — User entitled to policy groups, which may represent a human actor or an automated process. Each user must have a unique required user_id as data properties and a user type classification.\n"
        "- user type — Enumeration that classifies a user as a human user or a process user.\n"
        ...
        "- (user)-[:hasUserType]->(user type) — User is classified as a human user or process user.\n"
